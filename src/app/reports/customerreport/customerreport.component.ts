...
export class CustomerReportComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private initDataTable(): void {
		...
		this.dtOptions = {
			...
			columns: [
				...
				{
					title: this.app.localize('Gross Total'), data: 'grossTotal', orderable: false,
					render: (data: number) => this.app.formatCurrency(data)
				},
				{
					title: this.app.localize('Net Total'), data: 'netTotal', orderable: false,
				...
				{
					title: this.app.localize('Tips'), data: 'tipsGiven', orderable: false,
					render: (data: number) => `<strong class="text-info">${this.app.formatCurrency(data)}</strong>`
				},
				{
				...
				{
					title: this.app.localize('Loyalty Earned'), data: 'loyaltyPointsEarned', orderable: false,
					render: (data: number) => `<span class="badge badge-success">${this.app.formatNumber(data)}</span>`
				},
				{
					title: this.app.localize('Loyalty Redeemed'), data: 'loyaltyPointsRedeemed', orderable: false,
					render: (data: number) => `<span class="badge badge-warning">${this.app.formatNumber(data)}</span>`
				}
			],
			columnDefs: [
			...
			buttons: [
				{
					extend: 'excel',
					title: () => '',
					filename: () => 'CustomerReport_' + new Date().toISOString().slice(0, 10),
			...
			ajax: (params: any, callback: any) => {
				...
				this.http.get<any>('/api/reports/getcustomerreport', { params: query }).subscribe({
					next: response => {
	...
	private customizePdfExport(doc: any): void {
		...
		const infoLines = [
			{ text: this.app.localize('Customer Report'), bold: true, fontSize: 12, margin: [0, 0, 0, 4], alignment: 'center' },
	...
	private customizePrintExport(win: any): void {
		const infoHtml = `
			<div class="print-info mb-3" style="text-align:center; margin-bottom:10px; line-height:1.2;">
				<h5 class="text-primary mb-1" style="margin:0">${this.app.localize('Customer Report')}</h5>
