...
export class PurchaseReportComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private initDataTable(): void {
		...
		this.dtOptions = {
			...
			columns: [
				{
					...
					render: (data: any) => `<strong>${this.app.formatPeriod(data)}</strong>`
				},
				{
					title: this.app.localize('Purchases'), data: 'purchaseCount', orderable: false,
					render: (data: number) => `<span class="badge badge-primary">${data}</span>`
				},
				{
					title: this.app.localize('Sub Total'), data: 'subTotal', orderable: false,
					render: (data: number) => this.app.formatCurrency(data)
				},
				{
					title: this.app.localize('Discount'), data: 'discountTotal', orderable: false,
					render: (data: number) => `<span class="text-warning">${this.app.formatCurrency(data)}</span>`
				},
				{
					title: this.app.localize('Tax'), data: 'taxAmount', orderable: false,
					render: (data: number) => this.app.formatCurrency(data)
				},
				{
					title: this.app.localize('Shipping'), data: 'shippingTotal', orderable: false,
					render: (data: number) => this.app.formatCurrency(data)
				},
				{
					title: this.app.localize('Total'), data: 'totalAmount', orderable: false,
					render: (data: number) => `<strong>${this.app.formatCurrency(data)}</strong>`
				},
				{
					title: this.app.localize('Paid'), data: 'paidAmount', orderable: false,
					render: (data: number) => `<strong class="text-success">${this.app.formatCurrency(data)}</strong>`
				},
				{
					title: this.app.localize('Due'), data: 'dueAmount', orderable: false,
					render: (data: number) => `<strong class="${data > 0 ? 'text-danger' : 'text-muted'}">${this.app.formatCurrency(data)}</strong>`
				},
				{
					title: this.app.localize('Returns'), data: 'returnedAmount', orderable: false,
					render: (data: number) => `<span class="text-danger">${this.app.formatCurrency(data)}</span>`
				},
				{
					title: this.app.localize('Net Purchases'), data: 'netPurchases', orderable: false,
					render: (data: number) => `<strong class="text-success">${this.app.formatCurrency(data)}</strong>`
				}
			],
			columnDefs: [
			...
			buttons: [
				{
					extend: 'excel',
					title: () => '',
					filename: () => 'PurchaseReport_' + new Date().toISOString().slice(0, 10),
			...
			ajax: (params: any, callback: any) => {
				...
				this.http.get<any>('/api/reports/getpurchasereport', { params: query }).subscribe({
					next: response => {
						...
						this.info = {
							...
							startDate: this.app.formatDate(response.startDate) || '',
							endDate: this.app.formatDate(response.endDate) || '',
	...
	private customizePdfExport(doc: any): void {
		...
		const periodText = this.periodOptions.find(p => p.value === this.info.period)?.label || this.info.period;
		const infoLines = [
			{ text: this.app.localize('Purchase Report'), bold: true, fontSize: 12, margin: [0, 0, 0, 4], alignment: 'center' },
			{ text: `${this.info.locationName}`, fontSize: 9, margin: [0, 0, 0, 2], alignment: 'center' },
			{ text: `${periodText} - ${this.info.startDate} - ${this.info.endDate}`, fontSize: 9, margin: [0, 0, 0, 2], alignment: 'center' },
		];
	...
	private customizePrintExport(win: any): void {
		...
		const infoHtml = `
			<div class="print-info mb-3" style="text-align:center; margin-bottom:10px; line-height:1.2;">
				<h4 class="text-primary mb-1" style="margin:0">${this.app.localize('Purchase Report')}</h4>
				<p style="margin:0">${this.info.locationName}</p>
				<p style="margin:0">${periodText} - ${this.info.startDate} - ${this.info.endDate}</p>
			</div>`;
