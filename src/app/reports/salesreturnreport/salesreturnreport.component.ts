...
@Component({
	selector: 'app-salesreturnreport',
	templateUrl: './salesreturnreport.component.html',
	standalone: true,
	imports: [AppImports]
})
export class SalesReturnReportComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private initDataTable(): void {
		...
		this.dtOptions = {
			...
			columns: [
				...
				{
					title: this.app.localize('Refund Total'), data: 'refundTotal', orderable: false,
			...
			buttons: [
				{
					extend: 'excel',
					title: () => '',
					filename: () => 'SalesReturnReport_' + new Date().toISOString().slice(0, 10),
			...
			ajax: (params: any, callback: any) => {
				...
				this.http.get<any>('/api/reports/getsalesreturnreport', { params: query }).subscribe({
					next: response => {
						...
						this.info = {
							...
							period: response.period || this.filters.period
						};
	...
	private customizePdfExport(doc: any): void {
		...
		const infoLines = [
			{ text: this.app.localize('Sales Return Report'), bold: true, fontSize: 12, margin: [0, 0, 0, 4], alignment: 'center' },
	...
	private customizePrintExport(win: any): void {
		...
		const infoHtml = `
			<div class="print-info mb-3" style="text-align:center; margin-bottom:10px; line-height:1.2;">
				<h4 class="text-primary mb-1" style="margin:0">${this.app.localize('Sales Return Report')}</h4>
