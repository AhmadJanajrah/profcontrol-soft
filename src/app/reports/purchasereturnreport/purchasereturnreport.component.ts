...
export class PurchaseReturnReportComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private initDataTable(): void {
		...
		this.dtOptions = {
			...
			columns: [
				...
				{
					title: this.app.localize('Returns'), data: 'returnCount', orderable: false,
					render: (data: number) => `<span class="badge badge-danger">${data}</span>`
				},
				{
				...
				{
					title: this.app.localize('Charges'), data: 'returnCharges', orderable: false,
					render: (data: number) => this.app.formatCurrency(data)
				},
				{
					title: this.app.localize('Return Total'), data: 'returnTotal', orderable: false,
					render: (data: number) => `<strong class="text-danger">${this.app.formatCurrency(data)}</strong>`
				},
				{
					title: this.app.localize('Returned Amount'), data: 'returnedAmount', orderable: false,
					render: (data: number) => `<strong class="text-danger">${this.app.formatCurrency(data)}</strong>`
				}
			],
			columnDefs: [
			...
			buttons: [
				{
					extend: 'excel',
					title: () => '',
					filename: () => 'PurchaseReturnReport_' + new Date().toISOString().slice(0, 10),
			...
			ajax: (params: any, callback: any) => {
				...
				this.http.get<any>('/api/reports/getpurchasereturnreport', { params: query }).subscribe({
					next: response => {
	...
	private customizePdfExport(doc: any): void {
		...
		const infoLines = [
			{ text: this.app.localize('Purchase Return Report'), bold: true, fontSize: 12, margin: [0, 0, 0, 4], alignment: 'center' },
	...
	private customizePrintExport(win: any): void {
		...
		const infoHtml = `
			<div class="print-info mb-3" style="text-align:center; margin-bottom:10px; line-height:1.2;">
				<h4 class="text-primary mb-1" style="margin:0">${this.app.localize('Purchase Return Report')}</h4>
