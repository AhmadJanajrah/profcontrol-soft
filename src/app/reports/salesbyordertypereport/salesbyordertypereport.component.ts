...
@Component({
	selector: 'app-salesbyordertypereport',
	templateUrl: './salesbyordertypereport.component.html',
	standalone: true,
	imports: [AppImports]
})
export class SalesByOrderTypereportComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private initDataTable(): void {
		...
		this.dtOptions = {
			...
			columns: [
				{
					title: this.app.localize('Order Type'), data: 'orderType', orderable: false,
					render: (data: string) => `<strong class="badge badge-primary">${this.app.localize(data)}</strong>`
				},
				{
				...
				{
					title: this.app.localize('Discounts'), data: 'discounts', orderable: false,
					render: (data: number) => `<strong class="text-warning">${this.app.formatCurrency(data)}</strong>`
				},
				{
					title: this.app.localize('Taxes'), data: 'taxes', orderable: false,
					render: (data: number) => this.app.formatCurrency(data)
				},
				{
					title: this.app.localize('Tips'), data: 'tips', orderable: false,
			...
			buttons: [
				{
					extend: 'excel',
					title: () => '',
					filename: () => 'SalesByOrderTypeReport_' + new Date().toISOString().slice(0, 10),
			...
			ajax: (params: any, callback: any) => {
				...
				this.http.get<any>('/api/reports/getsalesbyordertypereport', { params: query }).subscribe({
					next: response => {
	...
	private customizePdfExport(doc: any): void {
		...
		const infoLines = [
			{ text: this.app.localize('Sales by Order Type Report'), bold: true, fontSize: 12, margin: [0, 0, 0, 4], alignment: 'center' },
	...
	private customizePrintExport(win: any): void {
		const infoHtml = `
			<div class="print-info mb-3" style="text-align:center; margin-bottom:10px; line-height:1.2;">
				<h4 class="text-primary mb-1" style="margin:0">${this.app.localize('Sales by Order Type Report')}</h4>
