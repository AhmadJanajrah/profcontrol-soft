...
export class SupplierReportComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private initDataTable(): void {
		...
		this.dtOptions = {
			...
			columns: [
				{
					title: this.app.localize('Supplier'), data: 'supplierName', orderable: false,
				...
				{
					title: this.app.localize('Total Purchase'), data: 'totalPurchaseAmount', orderable: false,
				...
				{
					title: this.app.localize('Returns'), data: 'returnAmount', orderable: false,
					render: (data: number) => `<strong class="text-danger">${this.app.formatCurrency(data)}</strong>`
				},
				{
					title: this.app.localize('Net Purchase'), data: 'netPurchaseAmount', orderable: false,
					render: (data: number) => `<strong class="text-success">${this.app.formatCurrency(data)}</strong>`
				},
				{
					title: this.app.localize('Avg Purchase'), data: 'averagePurchaseValue', orderable: false,
					render: (data: number) => this.app.formatCurrency(data)
				},
				{
					title: this.app.localize('Last Purchase'), data: 'lastPurchaseDate', orderable: false,
					render: (data: string) => this.app.formatDate(data)
				}
			],
			columnDefs: [
			...
			buttons: [
				{
					extend: 'excel',
					title: () => '',
					filename: () => 'SupplierReport_' + new Date().toISOString().slice(0, 10),
			...
			ajax: (params: any, callback: any) => {
				...
				this.http.get<any>('/api/reports/getsupplierreport', { params: query }).subscribe({
					next: response => {
	...
	private customizePdfExport(doc: any): void {
		...
		const infoLines = [
			{ text: this.app.localize('Supplier Report'), bold: true, fontSize: 12, margin: [0, 0, 0, 4], alignment: 'center' },
	...
	private customizePrintExport(win: any): void {
		const infoHtml = `
			<div class="print-info mb-3" style="text-align:center; margin-bottom:10px; line-height:1.2;">
				<h5 class="text-primary mb-1" style="margin:0">${this.app.localize('Supplier Report')}</h5>
