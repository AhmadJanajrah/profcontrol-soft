...
export class StaffReportComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private initDataTable(): void {
		...
		this.dtOptions = {
			...
			columns: [
				...
				{
					title: this.app.localize('Staff Member'), data: 'staffMember', orderable: false,
					render: (data: string) => `<span class="badge badge-info">${data}</span>`
				},
				{
				...
				{
					title: this.app.localize('Tips'), data: 'totalTips', orderable: false,
				...
				{
					title: this.app.localize('Avg Tip'), data: 'averageTipPerOrder', orderable: false,
					render: (data: number) => this.app.formatCurrency(data)
				}
			],
			columnDefs: [
			...
			buttons: [
				{
					extend: 'excel',
					title: () => '',
					filename: () => 'StaffPerformanceReport_' + new Date().toISOString().slice(0, 10),
			...
			ajax: (params: any, callback: any) => {
				...
				this.http.get<any>('/api/reports/getstaffperformancereport', { params: query }).subscribe({
					next: response => {
	...
	private customizePdfExport(doc: any): void {
		...
		const infoLines = [
			{ text: this.app.localize('Staff Performance Report'), bold: true, fontSize: 12, margin: [0, 0, 0, 4], alignment: 'center' },
	...
	private customizePrintExport(win: any): void {
		...
		const infoHtml = `
			<div class="print-info mb-3" style="text-align:center; margin-bottom:10px; line-height:1.2;">
				<h4 class="text-primary mb-1" style="margin:0">${this.app.localize('Staff Performance Report')}</h4>
