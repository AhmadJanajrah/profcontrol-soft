...
@Component({
	selector: 'app-itemsalesreport',
	templateUrl: './itemsalesreport.component.html',
	standalone: true,
	imports: [AppImports]
})
export class ItemSalesReportComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private initDataTable(): void {
		...
		this.dtOptions = {
			...
			columns: [
				{
					title: this.app.localize('Item Name'), data: 'itemName', orderable: false,
					render: (data: string) => `<strong>${data}</strong>`
				},
				{
					title: this.app.localize('Type'), data: 'itemType', orderable: false,
					render: (data: string) => `<span class="badge badge-info">${this.app.localize(data)}</span>`
				},
				{
					title: this.app.localize('Category'), data: 'category', orderable: false
				},
				{
					title: this.app.localize('Qty Sold'), data: 'quantitySold', orderable: false,
					render: (data: number) => `<span class="badge badge-primary">${this.app.formatNumber(data)}</span>`
				},
				{
				...
				{
					title: this.app.localize('Discounts'), data: 'discountAmount', orderable: false,
			...
			buttons: [
				{
					extend: 'excel',
					title: () => '',
					filename: () => 'ItemSalesReport_' + new Date().toISOString().slice(0, 10),
			...
			ajax: (params: any, callback: any) => {
				...
				this.http.get<any>('/api/reports/getitemsalesreport', { params: query }).subscribe({
					next: response => {
	...
	private customizePdfExport(doc: any): void {
		...
		const infoLines = [
			{ text: this.app.localize('Item Sales Report'), bold: true, fontSize: 12, margin: [0, 0, 0, 4], alignment: 'center' },
	...
	private customizePrintExport(win: any): void {
		const infoHtml = `
			<div class="print-info mb-3" style="text-align:center; margin-bottom:10px; line-height:1.2;">
				<h4 class="text-primary mb-1" style="margin:0">${this.app.localize('Item Sales Report')}</h4>
