...
export class SalesReportComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private initDataTable(): void {
		...
		this.dtOptions = {
			...
			columns: [
				...
				{
					title: this.app.localize('Discounts'), data: 'discountTotal', orderable: false,
					render: (data: number) => `<strong class="text-warning">${this.app.formatCurrency(data)}</strong>`
				},
				{
				...
				{
					title: this.app.localize('Charges'), data: 'chargeAmount', orderable: false,
					render: (data: number) => this.app.formatCurrency(data)
				},
				{
					title: this.app.localize('Tips'), data: 'tipAmount', orderable: false,
					render: (data: number) => `<span class="text-info">${this.app.formatCurrency(data)}</span>`
				},
				{
					title: this.app.localize('Fees'), data: 'processingFees', orderable: false,
					render: (data: number) => this.app.formatCurrency(data)
				},
				{
				...
				{
					title: this.app.localize('Gross Profit'), data: 'grossProfit', orderable: false,
					render: (data: number) => `<strong class="${data >= 0 ? 'text-success' : 'text-danger'}">${this.app.formatCurrency(data)}</strong>`
				},
				{
					title: this.app.localize('Avg Order'), data: 'averageOrderValue', orderable: false,
					render: (data: number) => this.app.formatCurrency(data)
				}
			],
			columnDefs: [
			...
			buttons: [
				{
					extend: 'excel',
					title: () => '',
					filename: () => 'SalesReport_' + new Date().toISOString().slice(0, 10),
			...
			ajax: (params: any, callback: any) => {
				...
				this.http.get<any>('/api/reports/getsalesreport', { params: query }).subscribe({
					next: response => {
	...
	private customizePdfExport(doc: any): void {
		...
		const infoLines = [
			{ text: this.app.localize('Sales Report'), bold: true, fontSize: 12, margin: [0, 0, 0, 4], alignment: 'center' },
	...
	private customizePrintExport(win: any): void {
		...
		const infoHtml = `
			<div class="print-info mb-3" style="text-align:center; margin-bottom:10px; line-height:1.2;">
				<h4 class="text-primary mb-1" style="margin:0">${this.app.localize('Sales Report')}</h4>
