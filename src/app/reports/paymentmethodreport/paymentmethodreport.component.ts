...
@Component({
	selector: 'app-paymentmethodreport',
	templateUrl: './paymentmethodreport.component.html',
	standalone: true,
	imports: [AppImports]
})
export class PaymentMethodReportComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private initDataTable(): void {
		...
		this.dtOptions = {
			...
			columns: [
				{
					title: this.app.localize('Payment Method'), data: 'paymentMethodName', orderable: false,
					render: (data: string) => `<strong class="badge badge-info">${data}</strong>`
				},
				{
					title: this.app.localize('Transactions'), data: 'transactionCount', orderable: false,
				...
				},
				{
					...
					render: (data: number) => `<span class="text-success">${this.app.formatCurrency(data)}</span>`
				},
				{
					title: this.app.localize('Refunds'), data: 'refundAmount', orderable: false,
					render: (data: number) => `<span class="text-danger">${this.app.formatCurrency(data)}</span>`
				},
				{
					title: this.app.localize('Net Amount'), data: 'netAmount', orderable: false,
					render: (data: number) => `<strong class="text-success">${this.app.formatCurrency(data)}</strong>`
				},
				{
					title: this.app.localize('Processing Fees'), data: 'processingFees', orderable: false,
					render: (data: number) => `<span class="text-warning">${this.app.formatCurrency(data)}</span>`
				},
				{
					title: this.app.localize('Tips'), data: 'tipsCollected', orderable: false,
					render: (data: number) => `<span class="text-info">${this.app.formatCurrency(data)}</span>`
				},
				{
					title: this.app.localize('Avg Transaction'), data: 'averageTransactionValue', orderable: false,
					render: (data: number) => this.app.formatCurrency(data)
				}
			],
			columnDefs: [
			...
			buttons: [
				{
					extend: 'excel',
					title: () => '',
					filename: () => 'PaymentMethodReport_' + new Date().toISOString().slice(0, 10),
			...
			ajax: (params: any, callback: any) => {
				...
				this.http.get<any>('/api/reports/getpaymentmethodreport', { params: query }).subscribe({
					next: response => {
	...
	private customizePdfExport(doc: any): void {
		...
		const infoLines = [
			{ text: this.app.localize('Sales by Payment Method Report'), bold: true, fontSize: 12, margin: [0, 0, 0, 4], alignment: 'center' },
	...
	private customizePrintExport(win: any): void {
		const infoHtml = `
			<div class="print-info mb-3" style="text-align:center; margin-bottom:10px; line-height:1.2;">
				<h4 class="text-primary mb-1" style="margin:0">${this.app.localize('Sales by Payment Method Report')}</h4>
