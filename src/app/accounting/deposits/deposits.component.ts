...
export class DepositsComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private initDataTable(): void {
		...
		this.dtOptions = {
			...
			columns: [
				...
				{ title: this.app.localize('Deposit Date'), data: 'depositDate', orderSequence: ['asc', 'desc'] },
			...
			ajax: (params: any, callback: any) => {
				...
				this.http.get<any>('/api/accounting/getdeposits', { params: query as any }).subscribe({
					next: response => {
						const formattedData = (response.data || []).map((item: any) => ({
							...item,
							depositDate: this.app.formatDate(item.depositDate),
	...
	private renderStatusBadge(status: number): string {
		// TransactionStatus: Draft=1, Posted=2, Cancelled=3
	...
	private loadFormData(): void {
		this.http.get<any>('/api/accounting/getdepositformdata').subscribe({
			next: response => {
	...
	private resetMainModal(): void {
		console.log(this.app.currentHTMLDate());
		this.deposit = {
			id: 0,
			locationId: this.app.getSelectedLocationId() || 0,
			bankAccountId: null,
			accountId: null,
			depositDate: this.app.currentHTMLDate(),
			amount: '',
	...
	public openMainModal(id: number): void {
		if (id > 0) {
			...
			this.http.get<any>(`/api/accounting/getdeposit/${id}`).subscribe({
				next: data => {
					...
					this.deposit = {
						...
						depositDate: this.app.APIDateTimeToHTMLDate(d.depositDate),
						amount: this.app.apiNumberToLocale(d.amount),
						status: d.status,
						notes: d.notes
					};
			...
			this.resetMainModal();
			this.mainModal.title = this.app.localize('Add Deposit');
			this.mainModal.show = true;
		}
	...
	public submitForm(form: NgForm): void {
		...
		const isUpdate = this.deposit.id > 0 && this.mainModal.isUpdate;
		const url = isUpdate
			? `/api/accounting/updatedeposit/${this.deposit.id}`
			: '/api/accounting/createdeposit';

		const payload = {
			...this.deposit,
			amount: this.app.localeToAPINumber(this.deposit.amount),
			depositDate: this.app.HTMLDateToAPIDateTime(this.deposit.depositDate)
		};
		...
		request$.subscribe({
			next: () => {
				const msg = isUpdate
					? this.app.localize('Deposit updated successfully.')
					: this.app.localize('Deposit created successfully.');
	...
	// Show confirmation dialog before deleting deposit
	...
	private delete(id: number): void {
		...
		this.http.delete<any>(`/api/accounting/deletedeposit/${id}`).subscribe({
			next: () => {
				this.app.showSuccessMessage(this.app.localize('Success!'), this.app.localize('Deposit deleted successfully.'));
				this.reloadDataTable();
