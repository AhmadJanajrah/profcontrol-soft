...
export class ChargesComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	// Apply To options (based on OrderType enum)
	...
	constructor(
		...
		this.resetMainModal();
		this.applyToOptions.forEach(opt => {
	...
	private initDataTable(): void {
		...
		this.dtOptions = {
			...
			columns: [
				...
				{ title: this.app.localize('Charge Name'), data: 'chargeName', orderSequence: ['asc', 'desc'] },
				{ title: this.app.localize('Charge Value'), data: 'chargeValue', orderable: false },
				{ title: this.app.localize('Description'), data: 'description', orderable: false },
				{ title: this.app.localize('Apply To'), data: 'applyTo', orderable: false },
				{
					title: this.app.localize('Default'),
					data: null,
					orderable: false,
					render: (data: any) => this.renderDefaultColumn(data)
				},
				{
			...
			ajax: (params: any, callback: any) => {
				...
				this.http.get<any>('/api/settings/getcharges', { params: query }).subscribe({
					next: response => {
						const formattedData = (response.data || []).map((item: any) => ({
							...item,
							chargeValue: item.isPercentage ? `${this.app.formatPercent(item.chargeValue)}` : this.app.formatCurrency(item.chargeValue),
							applyTo: this.getApplyToLabel(item.applyTo),
	...
	private renderDefaultColumn(isDefault: boolean): string {
		return isDefault
	...
	private renderActionColumn(row: any): string {
		...
		<div class="dropdown-menu dropdown-menu-end">
			...
			<a class="dropdown-item text-danger delete-button" data-id="${row.id}" data-name="${row.chargeName}" href="javascript:void(0);">
	...
	private addTableEventListeners(): void {
		...
		if (tableBody) {
			this.renderer.listen(tableBody, 'click', (event: Event) => {
				...
				if (editButton) {
					const chargeId = parseInt(editButton.getAttribute('data-id') || '0');
					this.openMainModal(chargeId);
				}

				if (deleteButton) {
					const chargeId = parseInt(deleteButton.getAttribute('data-id') || '0');
					const chargeName = deleteButton.getAttribute('data-name') || '';
					this.confirmDeleteCharge(chargeId, chargeName);
				}
			});
		}
	}
	...
	// Open charge modal for add/edit
	public openMainModal(id: number): void {
		if (id > 0) {
			this.resetMainModal();
			this.mainModal.loading = true;
			this.mainModal.title = this.app.localize('Edit Charge');
			this.mainModal.btnSaveText = this.app.localize('Update');
			this.mainModal.show = true;
			this.mainModal.isUpdate = true;

			this.http.get<any>(`/api/settings/getcharge/${id}`).subscribe({
				next: data => {
					this.charge = { ...data.charge };
					this.charge.chargeValue = this.app.apiNumberToLocale(this.charge.chargeValue);
			...
			this.resetMainModal();
			this.mainModal.title = this.app.localize('Add Charge');
			this.mainModal.show = true;
		}
	...
	public submitForm(form: NgForm): void {
		...
		const isUpdate = this.charge.id > 0 && this.mainModal.isUpdate;
		const url = isUpdate
			? `/api/settings/updatecharge/${this.charge.id}`
			: '/api/settings/createcharge';

		const chargeData = {
			...this.charge,
			chargeValue: this.app.localeToAPINumber(this.charge.chargeValue)
		};

		const request$ = isUpdate
			? this.http.put<any>(url, chargeData)
			: this.http.post<any>(url, chargeData);

		request$.subscribe({
			next: () => {
				const msg = isUpdate
					? this.app.localize('Charge updated successfully.')
					: this.app.localize('Charge created successfully.');
	...
	// Show confirmation dialog before deleting charge
	private confirmDeleteCharge(id: number, chargeName: string): void {
		const message = `${this.app.localize('Are you sure you want to delete')} <strong>"${chargeName}"</strong>?<br>
	...
	// Delete charge by id
	private deleteCharge(id: number): void {
		var dialogId = this.app.showLoadingDialog('Deleting...');
		this.http.delete<any>(`/api/settings/deletecharge/${id}`).subscribe({
			next: () => {
				this.app.showSuccessMessage(this.app.localize('Success!'), this.app.localize('Charge deleted successfully.'));
				this.reloadDataTable();
	...
	// Get apply to label
	public getApplyToLabel(value: number): string {
		const option = this.applyToOptions.find(opt => opt.value === value);
