...
export class DiscountsComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private initDataTable(): void {
		...
		this.dtOptions = {
			...
			columns: [
				...
				{ title: this.app.localize('Discount Name'), data: 'discountName', orderSequence: ['asc', 'desc'] },
				{ title: this.app.localize('Discount Value'), data: 'discountValue', orderable: false },
			...
			ajax: (params: any, callback: any) => {
				...
				this.http.get<any>('/api/settings/getdiscounts', { params: query }).subscribe({
					next: response => {
	...
	private renderActionColumn(row: any): string {
		...
		<div class="dropdown-menu dropdown-menu-end">
			...
			<a class="dropdown-item text-danger delete-button" data-id="${row.id}" data-name="${row.discountName}" href="javascript:void(0);">
	...
	private addTableEventListeners(): void {
		...
		if (tableBody) {
			this.renderer.listen(tableBody, 'click', (event: Event) => {
				...
				if (editButton) {
					const discountId = parseInt(editButton.getAttribute('data-id') || '0');
					this.openMainModal(discountId);
				}

				if (deleteButton) {
					const discountId = parseInt(deleteButton.getAttribute('data-id') || '0');
					const discountName = deleteButton.getAttribute('data-name') || '';
					this.confirmDeleteDiscount(discountId, discountName);
				}
			});
		}
	}
	...
	// Open discount modal for add/edit
	public openMainModal(id: number): void {
		if (id > 0) {
			this.resetMainModal();
			this.mainModal.loading = true;
			this.mainModal.title = this.app.localize('Edit Discount');
			this.mainModal.btnSaveText = this.app.localize('Update');
			this.mainModal.show = true;
			this.mainModal.isUpdate = true;

			this.http.get<any>(`/api/settings/getdiscount/${id}`).subscribe({
				next: data => {
					this.discount = { ...data.discount };
					this.discount.discountValue = this.app.apiNumberToLocale(this.discount.discountValue);
			...
			this.resetMainModal();
			this.mainModal.title = this.app.localize('Add Discount');
			this.mainModal.show = true;
		}
	...
	public submitForm(form: NgForm): void {
		...
		const isUpdate = this.discount.id > 0 && this.mainModal.isUpdate;
		const url = isUpdate
			? `/api/settings/updatediscount/${this.discount.id}`
			: '/api/settings/creatediscount';

		const discountData = {
			...this.discount,
			discountValue: this.app.localeToAPINumber(this.discount.discountValue)
		};

		const request$ = isUpdate
			? this.http.put<any>(url, discountData)
			: this.http.post<any>(url, discountData);

		request$.subscribe({
			next: () => {
				const msg = isUpdate
					? this.app.localize('Discount updated successfully.')
					: this.app.localize('Discount created successfully.');
	...
	// Show confirmation dialog before deleting discount
	private confirmDeleteDiscount(id: number, discountName: string): void {
		const message = `${this.app.localize('Are you sure you want to delete')} <strong>"${discountName}"</strong>?<br>
	...
	private deleteDiscount(id: number): void {
		...
		this.http.delete<any>(`/api/settings/deletediscount/${id}`).subscribe({
			next: () => {
				this.app.showSuccessMessage(this.app.localize('Success!'), this.app.localize('Discount deleted successfully.'));
				this.reloadDataTable();
