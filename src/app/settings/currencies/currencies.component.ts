...
export class CurrenciesComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private initDataTable(): void {
		...
		this.dtOptions = {
			...
			columns: [
				...
				{ title: this.app.localize('Code'), data: 'currencyCode', orderSequence: ['asc', 'desc'] },
				{ title: this.app.localize('Name'), data: 'currencyName', orderSequence: ['asc', 'desc'] },
				{ title: this.app.localize('Symbol'), data: 'symbol', orderable: false },
				{ title: this.app.localize('Exchange Rate'), data: 'exchangeRate', orderable: false },
				{ title: this.app.localize('Decimal Places'), data: 'decimalPlaces', orderable: false },
				{
					title: this.app.localize('Base Currency'),
					data: 'isBaseCurrency',
					orderable: false,
					render: (data: boolean) => this.renderIsBaseCurrency(data)
				},
				{
			...
			ajax: (params: any, callback: any) => {
				...
				this.http.get<any>('/api/settings/getcurrencies', { params: query }).subscribe({
					next: response => {
						const formattedData = (response.data || []).map((item: any) => ({
							...item,
							exchangeRate: this.app.formatCurrency(item.exchangeRate),
							lastRateUpdate: this.app.formatDateTime(item.lastRateUpdate) || `—`,
						}));
						callback({
	...
	// Render isBaseCurrency badge
	private renderIsBaseCurrency(data: boolean): string {
		return `
			<span class="badge ${data ? 'badge-primary' : 'badge-light'}">
				${data ? this.app.localize('Yes') : this.app.localize('No')}
			</span>
		`;
	}

	// Render status badge
	private renderStatusBadge(data: boolean): string {
		return `
			<span class="badge ${data ? 'badge-primary' : 'badge-danger'}">${data ? 'Active' : 'Inactive'}</span>
		`;
	}
	...
	private renderActionColumn(row: any): string {
		...
		if (!row.isBaseCurrency) {
			actions += `
			<a class="dropdown-item text-danger delete-button" data-id="${row.id}" data-name="${row.currencyName}" href="javascript:void(0);">
	...
	private addTableEventListeners(): void {
		...
		if (tableBody) {
			this.renderer.listen(tableBody, 'click', (event: Event) => {
				...
				if (editButton) {
					const currencyId = parseInt(editButton.getAttribute('data-id') || '0');
					this.openMainModal(currencyId);
				}

				if (deleteButton) {
					const currencyId = parseInt(deleteButton.getAttribute('data-id') || '0');
					const currencyName = deleteButton.getAttribute('data-name') || '';
					this.confirmDeleteCurrency(currencyId, currencyName);
				}
			});
		}
	}
	...
	// Load system currencies for selection
	private loadSystemCurrencies(): void {
		this.http.get<any>('/api/settings/getcurrencyformdata').subscribe({
			next: response => {
				this.systemCurrencies = response.currencies;
			},
			error: error => {
				this.app.handleApiError(error);
			}
	...
	// Open currency modal for add/edit
	public openMainModal(id: number): void {
		if (id > 0) {
			this.resetMainModal();
			this.mainModal.loading = true;
			this.mainModal.title = this.app.localize('Edit Currency');
			this.mainModal.btnSaveText = this.app.localize('Update');
			this.mainModal.show = true;
			this.mainModal.isUpdate = true;

			this.http.get<any>(`/api/settings/getcurrency/${id}`).subscribe({
				next: data => {
					this.currency = { ...data.currency };
					this.currency.exchangeRate = this.app.apiNumberToLocale(this.currency.exchangeRate);
			...
			this.resetMainModal();
			this.mainModal.title = this.app.localize('Add Currency');
			this.mainModal.show = true;
		}
	...
	public submitForm(form: NgForm): void {
		...
		const isUpdate = this.currency.id > 0 && this.mainModal.isUpdate;
		const url = isUpdate
			? `/api/settings/updatecurrency/${this.currency.id}`
			: '/api/settings/createcurrency';

		const currencyData = {
			...this.currency,
			exchangeRate: this.app.localeToAPINumber(this.currency.exchangeRate)
		};

		const request$ = isUpdate
			? this.http.put<any>(url, currencyData)
			: this.http.post<any>(url, currencyData);

		request$.subscribe({
			next: () => {
				const msg = isUpdate
					? this.app.localize('Currency updated successfully.')
					: this.app.localize('Currency created successfully.');
	...
	private confirmDeleteCurrency(id: number, currencyName: string): void {
		const message = `${this.app.localize('Are you sure you want to delete')} <strong>"${currencyName}"</strong>?<br>
	...
	private deleteCurrency(id: number): void {
		var dialogId = this.app.showLoadingDialog('Deleting...');
		this.http.delete<any>(`/api/settings/deletecurrency/${id}`).subscribe({
			next: () => {
				this.app.showSuccessMessage(this.app.localize('Success!'), this.app.localize('Currency deleted successfully.'));
				this.reloadDataTable();
