...
export class ShiftsComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private initDataTable(): void {
		...
		this.dtOptions = {
			...
			columns: [
				...
				{ title: this.app.localize('Shift Name'), data: 'shiftName', orderSequence: ['asc', 'desc'] },
				{ title: this.app.localize('Timing'), data: 'timing', orderable: false },
				{ title: this.app.localize('Total Hours'), data: 'totalHours', orderable: false },
				{ title: this.app.localize('Break Minutes'), data: 'breakMinutes', orderable: false },
			...
			ajax: (params: any, callback: any) => {
				...
				this.http.get<any>('/api/hrm/getshifts', { params: query }).subscribe({
					next: response => {
						const formattedData = (response.data || []).map((item: any) => ({
							...item,
							timing: this.app.formatTime(item.startTime) + ' - ' + this.app.formatTime(item.endTime),
							totalHours: item.totalHours ? `${item.totalHours.toFixed(1)}h` : '—',
							breakMinutes: item.breakMinutes ? `${item.breakMinutes} min` : '0 min',
	...
	private renderActionColumn(row: any): string {
		...
		<div class="dropdown-menu dropdown-menu-end">
			...
			<a class="dropdown-item text-danger delete-button" data-id="${row.id}" data-name="${row.shiftName}" href="javascript:void(0);">
	...
	private addTableEventListeners(): void {
		...
		if (tableBody) {
			this.renderer.listen(tableBody, 'click', (event: Event) => {
				...
				if (editButton) {
					const shiftId = parseInt(editButton.getAttribute('data-id') || '0');
					this.openMainModal(shiftId);
				}

				if (deleteButton) {
					const shiftId = parseInt(deleteButton.getAttribute('data-id') || '0');
					const shiftName = deleteButton.getAttribute('data-name') || '';
					this.confirmDeleteShift(shiftId, shiftName);
				}
			});
		}
	}
	...
	public openMainModal(id: number): void {
		if (id > 0) {
			...
			this.mainModal.title = this.app.localize('Edit Shift');
			this.mainModal.btnSaveText = this.app.localize('Update');
			this.mainModal.show = true;
			this.mainModal.isUpdate = true;

			this.http.get<any>(`/api/hrm/getshift/${id}`).subscribe({
				next: data => {
					this.shift = { ...data.shift };
					// Convert UTC times to local for editing
					if (this.shift.startTime) {
						this.shift.startTime = this.app.APITimeToHTMLTime(this.shift.startTime);
					}
					if (this.shift.endTime) {
						this.shift.endTime = this.app.APITimeToHTMLTime(this.shift.endTime);
					}
			...
			this.resetMainModal();
			this.mainModal.title = this.app.localize('Add Shift');
			this.mainModal.show = true;
		}
	...
	public submitForm(form: NgForm): void {
		...
		const isUpdate = this.shift.id > 0 && this.mainModal.isUpdate;
		const url = isUpdate
			? `/api/hrm/updateshift/${this.shift.id}`
			: '/api/hrm/createshift';

		// Prepare data with time conversion
		const shiftData = { ...this.shift };
		if (shiftData.startTime) {
			shiftData.startTime = this.app.HTMLTimeToAPITime(shiftData.startTime);
		}
		if (shiftData.endTime) {
			shiftData.endTime = this.app.HTMLTimeToAPITime(shiftData.endTime);
		}

		const request$ = isUpdate
			? this.http.put<any>(url, shiftData)
			: this.http.post<any>(url, shiftData);

		request$.subscribe({
			next: () => {
				const msg = isUpdate
					? this.app.localize('Shift updated successfully.')
					: this.app.localize('Shift created successfully.');
	...
	// Show confirmation dialog before deleting shift
	private confirmDeleteShift(id: number, shiftName: string): void {
		const message = `${this.app.localize('Are you sure you want to delete')} <strong>"${shiftName}"</strong>?<br>
	...
	// Delete shift by id
	private deleteShift(id: number): void {
		var dialogId = this.app.showLoadingDialog('Deleting...');
		this.http.delete<any>(`/api/hrm/deleteshift/${id}`).subscribe({
			next: () => {
				this.app.showSuccessMessage(this.app.localize('Success!'), this.app.localize('Shift deleted successfully.'));
				this.reloadDataTable();
