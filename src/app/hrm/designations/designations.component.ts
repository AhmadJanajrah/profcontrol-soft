...
export class DesignationsComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	private initDataTable(): void {
		...
		this.dtOptions = {
			...
			columns: [
				...
				{ title: this.app.localize('Designation Name'), data: 'designationName', orderSequence: ['asc', 'desc'] },
				{ title: this.app.localize('Department'), data: 'departmentName', orderSequence: ['asc', 'desc'] },
				{ title: this.app.localize('Salary Grade'), data: 'salaryGrade', orderable: false },
			...
			ajax: (params: any, callback: any) => {
				...
				this.http.get<any>('/api/hrm/getdesignations', { params: query }).subscribe({
					next: response => {
	...
	private renderActionColumn(row: any): string {
		...
		<div class="dropdown-menu dropdown-menu-end">
			...
			<a class="dropdown-item text-danger delete-button" data-id="${row.id}" data-name="${row.designationName}" href="javascript:void(0);">
	...
	private addTableEventListeners(): void {
		...
		if (tableBody) {
			this.renderer.listen(tableBody, 'click', (event: Event) => {
				...
				if (editButton) {
					const designationId = parseInt(editButton.getAttribute('data-id') || '0');
					this.openMainModal(designationId);
				}

				if (deleteButton) {
					const designationId = parseInt(deleteButton.getAttribute('data-id') || '0');
					const designationName = deleteButton.getAttribute('data-name') || '';
					this.confirmDeleteDesignation(designationId, designationName);
				}
			});
		}
	}
	...
	private loadFormData(): void {
		this.http.get<any>('/api/hrm/getdesignationformdata').subscribe({
			next: response => {
	...
	public openMainModal(id: number): void {
		if (id > 0) {
			...
			this.resetMainModal();
			this.mainModal.title = this.app.localize('Add Designation');
			this.mainModal.show = true;
		}
	...
	public submitForm(form: NgForm): void {
		...
		const isUpdate = this.designation.id > 0 && this.mainModal.isUpdate;
		const url = isUpdate
			? `/api/hrm/updatedesignation/${this.designation.id}`
			: '/api/hrm/createdesignation';

		const request$ = isUpdate
			? this.http.put<any>(url, this.designation)
			: this.http.post<any>(url, this.designation);

		request$.subscribe({
			next: () => {
				const msg = isUpdate
					? this.app.localize('Designation updated successfully.')
					: this.app.localize('Designation created successfully.');
	...
	// Show confirmation dialog before deleting designation
	private confirmDeleteDesignation(id: number, designationName: string): void {
		const message = `${this.app.localize('Are you sure you want to delete')} <strong>"${designationName}"</strong>?<br>
	...
	private deleteDesignation(id: number): void {
		var dialogId = this.app.showLoadingDialog('Deleting...');
		this.http.delete<any>(`/api/hrm/deletedesignation/${id}`).subscribe({
			next: () => {
				this.app.showSuccessMessage(this.app.localize('Success!'), this.app.localize('Designation deleted successfully.'));
				this.reloadDataTable();
